    ...
    async execute(command: RefreshTokensCommand){
      try{
        ...
        if(device) {
          ...
          const expiresAt = new Date().getTime() + 300000
          ...
          const accessToken = this.jwtService.sign(payloadAccess, {expiresIn: '2m'})
          const refreshToken = this.jwtService.sign(payloadRefresh, {expiresIn: '5m'})
