...
describe('AppController', () => {
  ...
  describe('post-like-controller', () => {
    ...
    it('should return 404 for creating like by incorrect post', async () => {
        await request(server).put(`/posts/${constants.variables.incorrectAnyObjectId}/like-status`).set('Authorization', `Bearer ${constants.variables.accessToken}`).send(constants.like).expect(404)
    })

    it('should return 400 for creating like by incorrect input-data', async () => {
        await request(server).put(`/posts/${constants.variables.incorrectAnyObjectId}/like-status`).set('Authorization', `Bearer ${constants.variables.accessToken}`).send().expect(400)
    })

    it('should return 401 for creating like without auth', async () => {
        await request(server).put(`/posts/${constants.variables.incorrectAnyObjectId}/like-status`).send(constants.like).expect(401)
    })
